package com.attendease.backend.controller.authentication.biometrics;

import com.attendease.backend.data.dto.request.FacialRegistrationRequest;
import com.attendease.backend.data.dto.response.FaceEncodingResponse;
...
import com.attendease.backend.services.authentication.biometrics.FacialAuthenticationService;
...
@Slf4j
public class FacialAuthenticationController {

    private final FacialAuthenticationService facialAuthenticationService;
    ...
    public FacialAuthenticationController(FacialAuthenticationService facialAuthenticationService,HttpHeaders httpHeaders,RestTemplate restTemplate) {
        this.facialAuthenticationService = facialAuthenticationService;
    ...
    public ResponseEntity<String> registerFacialData(@PathVariable String studentNumber,@Valid @RequestBody FacialRegistrationRequest request) {
        try {
            ...
            try {
                response = restTemplate.postForEntity(FACIAL_RECOGNITION_SERVICE_API_BASE_URL + FACIAL_RECOGNITION_SERVICE_API_VALIDATE_FACIAL_ENCODING_ENDPOINT, requestEntity, FaceEncodingResponse.class);
            } catch (Exception e) {
            ...
            FaceEncodingResponse responseBody = response.getBody();
            ...
            Students student = facialAuthenticationService.getStudentByStudentNumber(studentNumber);
            String result = facialAuthenticationService.registerNewFacialBiometrics(student, faceEncodingList);
    ...
    public ResponseEntity<String> getFacialStatus(@PathVariable String studentNumber) {
        try {
            Optional<BiometricData> biometricData = facialAuthenticationService.getFacialStatus(studentNumber);
    ...
    public ResponseEntity<String> recalibrateFacialData(@PathVariable String studentNumber) {
        try {
            facialAuthenticationService.recalibrateFacialBiometrics(studentNumber);
